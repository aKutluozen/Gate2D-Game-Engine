...
var Controls = (function () {
    ...
    return {
        /**
         * Sends mouse or touch coordinates to whoever is listening
         * 
        ...
        initMouseControls: function (entities) {
            ...
            Video.canvas().addEventListener('touchstart', function (event) {
                for (let i = 0; i < entities.length; i++) {
                    if (entities[i].controlled) {
        ...
        getTouchPosition: function (event) {
            event.preventDefault();
            return {
                x: event.touches[0].pageX - Video.canvas().offsetLeft,
                y: event.touches[0].pageY - Video.canvas().offsetTop
            }
        },
